...
DBEH = {
    ...
    'DTI_P': {'DIS_IND_P': {'DI_P_TABLE': ['Autores', 'Contrato (si aplica)', 'Nombre comercial (si aplica)', 'Agregue las evidencias verificadas al repositorio digital y genere un hipervínculo en este campo', '¿El producto cumple con los requisitos para ser avalado?']},
              ...
              # add
              'NUTRA_P': {'NUTRA_P_TABLE': ['Autores', 'Agregue la evidencia verificada al repositorio digital y copie el link del archivo en este campo', '¿El producto cumple con los requisitos para ser avalado?']},
...
def clean_df(df):
    ...
    c = [x for x in df.columns if x.find(
        'Unnamed:') == -1 and x.find('Revisar') == -1 and x.find('Avalar integrante') == -1]
    dfc = df[c]
    return dfc
...
def format_df(df, sheet_name, start_row, writer, eh, veh=None):
    ...
    m_range = d.get(str(start)) + str(start_row + 1) + ':' + \
        d.get(str(end)) + str(start_row + 1)

    worksheet.merge_range(
        m_range, 'Información suministrada por la Vicerrectoría de Investigación', merge_format)

    # for merge headers cells
    _m_range = d.get(str(end + 1)) + str(start_row + 1) + ':' + \
        d.get(str(end + len(eh))) + str(start_row + 1)

    worksheet.merge_range(
        _m_range, 'Validación del Centro, Instituto o Corporación', merge_format)
    ...
    v_range = 'A' + str(start_row + 3) + ':' + 'A' + \
        str(df.shape[0] + start_row + 2)
    worksheet.data_validation(v_range, {'validate': 'list',
                                        'source': ['Sí', 'No']})

    if sheet_name != '3.Integrantes grupo':

        v_range = d.get(str(end + len(eh))) + str(start_row + 3) + ':' + \
            d.get(str(end + len(eh))) + str(df.shape[0] + start_row + 2)
        worksheet.data_validation(v_range, {'validate': 'list',
                                  'source': ['Sí', 'No']})

    # Integrantes
    if veh == 0:
        v_range = d.get(str(end + len(eh) - 2)) + str(start_row + 3) + ':' + \
            d.get(str(end + len(eh) - 2)) + str(df.shape[0] + start_row + 2)
        worksheet.data_validation(v_range, {'validate': 'list',
                                            'source': ['Sí', 'No']})
    # patentes
    if veh == 1:
        v_range = d.get(str(end + len(eh) - 3)) + str(start_row + 3) + ':' + \
            d.get(str(end + len(eh) - 3)) + str(df.shape[0] + start_row + 2)
        worksheet.data_validation(v_range, {'validate': 'list',
                                  'source': ['Sí', 'No']})
        v_range = d.get(str(end + len(eh) - 4)) + str(start_row + 3) + ':' + \
            d.get(str(end + len(eh) - 4)) + str(df.shape[0] + start_row + 2)
        worksheet.data_validation(v_range, {'validate': 'list',
                                  'source': ['Sí', 'No']})
        v_range = d.get(str(end + len(eh) - 5)) + str(start_row + 3) + ':' + \
            d.get(str(end + len(eh) - 5)) + str(df.shape[0] + start_row + 2)
        worksheet.data_validation(v_range, {'validate': 'list',
                                  'source': ['Sí', 'No']})
    if veh == 2:
        v_range = d.get(str(end + len(eh) - 2)) + str(start_row + 3) + ':' + \
...
def format_info(df, writer, sheet_name):
    ...
    # merge d3:f3
    worksheet.merge_range(
        'D3:F3', ' Datos de identificación y contacto', title3)

    # D5: F5
    worksheet.merge_range(
        'D5:E5', 'Número inscripcion a la convocatoria:', merge_format)
    ...
    # d9:f9
    worksheet.merge_range(
        'D10:F10', 'Identificación del Centro de Investigación', merge_format)
    # write
    ...
    # d13:f13
    worksheet.merge_range(
        'D13:F13', 'Identificación de quien diligencia el formato', merge_format)
...
def format_ptt(workbook):
    ...
    datos = clean_df(pd.read_excel(
        'https://github.com/restrepo/InstituLAC/raw/main/data/template_data.xlsx'))
    ...
    abstract = workbook.add_format(
        {'font_size': 20, 'center_across': True, 'text_wrap': True})
    normal = workbook.add_format({'font_size': 12, 'text_wrap': True})

    # ***************
    # Creates the first work-sheet
    # IMPORTANT → worksheet is the same object  used in the official document
    # at https://xlsxwriter.readthedocs.io
    ...
    datos.to_excel(writer, sheet_name="1.Presentación",
                   startrow=12, startcol=2, index=False)
    # **************************************************
...
def login(user, password, sleep=0.8, headless=True):
    ...
    # login
    browser = h.start_firefox(
        'https://scienti.minciencias.gov.co/institulac2-war/', headless=headless)

    time.sleep(sleep)
    h.click('Consulte Aquí')

    time.sleep(sleep)
    h.write('UNIVERSIDAD DE ANTIOQUIA',
            into='Digite el nombre de la Institución')  # name ins
    ...
    h.click(browser.find_element_by_xpath(
        '//table[@id="grupos_avalados"]//select[@name="maxRows"]'))

    time.sleep(sleep)
    h.select(browser.find_element_by_xpath(
        '//table[@id="grupos_avalados"]//select[@name="maxRows"]'), '100')
    return browser
...
def get_groups(browser, DIR='InstituLAC', sleep=0.8):
    ...
    while cont:
        ...
        try:
            ...
            time.sleep(sleep)
            df = pd.read_html(
                source_g, attrs={"id": "grupos_avalados"}, header=2)[0]
            ...
            # catch urls
            url = [a.get_attribute('href') for a in browser.find_elements_by_xpath(
                '//table[@id="grupos_avalados"]//td[5]/a')]
            dfgp['Revisar'] = url
            dfg = dfg.append(dfgp)

            # click next page. this instruction rise error of the end.
            h.click(browser.find_element_by_xpath(
                '//table[@id="grupos_avalados"]//tr/td[3]/a'))
...
def get_DB(browser, DB=[], dfg=pd.DataFrame(), sleep=0.8,
           DIR='InstituLAC', start=None, end=None, start_time=0):
    ...
    for idx in dfg.index[start:end]:       # TEST
        ...
        h.wait_until(lambda: browser.find_element_by_xpath(
            '//td[@id="bodyPrincipal"]//a[text()="Ver productos"]') is not None)
        h.click(browser.find_element_by_xpath(
            '//td[@id="bodyPrincipal"]//a[text()="Ver productos"]'))

        # products by belongs to  # time time time
        # time.sleep(sleep*7)       # time time time
        h.wait_until(lambda: browser.find_element_by_xpath(
            '//*[@id="ProdsPertenecia"]') is not None)
        ...
        for i in browser.find_elements_by_xpath(
            ...
            for j in browser.find_elements_by_xpath(
                    '//div[@aria-labelledby="%s"]/h3' % i.get_attribute('id')):
                ...
                if cat_ > 0 and pro_ > 0:
                    ...
                    list_of_prods.append(
                        [i.get_attribute('id'), j.get_attribute('id')])
        ...
        if (cat_ > 0 and L > 0):
            ...
            list_of_prods.append(
                [_NC.get_attribute('id'), LIB.get_attribute('id')])

        if (cat_ > 0 and CL > 0):

            list_of_prods.append(
                [_NC.get_attribute('id'), CAP_LIB.get_attribute('id')])
        ...
        for p in range(len(list_of_prods)):
            ...
            if list_of_prods[p][0] == 'NC':
                ...
                query = 'categoria=%s&subcategoria=%s&aval=T' % (
                    list_of_prods[p][0], list_of_prods[p][1])

            else:

                query = 'categoriaP=%s&subcategoriaP=%s&aval=P' % (
                    list_of_prods[p][0], list_of_prods[p][1])

            # make url query
            url_query = url_products.split(
                '?')[0] + '?' + query + '&' + url_products.split('?')[1]
            ...
            if isinstance(table_id, str):  # case one table
                ...
                time.sleep(sleep * 2)
                try:
                    table = pd.read_html(page_source, attrs={
                                         'id': table_id}, header=2)[0][1:-1]
                except ValueError:
                    table = None
                ...
                for i in range(len(table_id)):
                    ...
                    if list_of_prods[p][1] == 'DC_P' and i == 3:
                        ...
                        title_table = browser.find_elements_by_xpath(
                            '//div/p[@class="titulo_tabla"]')[i].text

                        # catch table software
                        table = pd.read_html(page_source, attrs={
                                             'id': table_id[i]}, header=2)[1][1:-1]

                        # store table
                        DBG[list_of_prods[p][0]][list_of_prods[p]
                                                 [1]]['DC_DES_P_TABLE'] = table
...
def to_excel(DB, dfg, DIR='InstituLAC'):
    ...
    for idxx in range(len(DB)):
        ...
        os.makedirs(
            f'{DIR}/{cod_gr}/Repositorio_digital_{cod_gr}', exist_ok=True)
        writer = pd.ExcelWriter(
            f'{DIR}/{cod_gr}/{name}{cod_gr}.xlsx', engine='xlsxwriter')
        ...
        try:
            df = rename_col(clean_df(
                DBG['NC_P']['LIB_P']['LIB_P_TABLE']), 'Título del artículo', 'Título del libro')
        ...
        try:
            df = rename_col(clean_df(
                DBG['NC']['LIB']['LIB_T_AVAL_TABLE']), 'Título del artículo', 'Título del libro')
        ...
        # libros formacion
        try:
            df = rename_col(clean_df(DBG['ASC_P']['GEN_CONT_IMP_P']['GC_I_P_TABLE_5']),
                            'Título del libro', 'Título del libro formación')  # lib form
        ...
        try:
            df = clean_df(DBG['NC']['CAP_LIB']
                          ['CAP_LIB_T_AVAL_TABLE'])  # ,veh = 2
        ...
        try:
            df = rename_col(clean_df(DBG['NC_P']['TRA_FIL_P']['TRA_FIL_P_TABLE']),
                            'Título del libro', 'Título traducción filologica')
        ...
        # patentes
        try:
            df = rename_col(clean_df(DBG['NC_P']['PAT_P']['PAT_P_TABLE']),
                            'Título del artículo', 'Título de la patente')  # veh=1
        ...
        try:
            df = clean_df(DBG['NC_P']['PRD_INV_ART_P']
                          ['PAAD_P_TABLE'])  # veh = 1
        ...
        try:
            ...
            df = rename_col(clean_df(DBG['DTI_P']['DIS_IND_P']['DI_P_TABLE']),
                            'Nombre del diseño', 'Nombre del diseño industrial')
        ...
        try:
            df = rename_col(clean_df(DBG['DTI_P']['CIR_INT_P']['ECI_P_TABLE']),
                            'Nombre del diseño', 'Nombre del diseño circuito')
        ...
        # software
        try:
            df = rename_col(clean_df(DBG['DTI_P']['SOFT_P']['SF_P_TABLE']),
                            'Nombre del diseño', 'Nombre del diseño de software')
        ...
        try:
            df = rename_col(clean_df(
                DBG['DTI_P']['SEC_IND_P']['SE_P_TABLE']), 'Producto', 'Nombre secreto industrial')
        ...
        try:
            df = rename_col(clean_df(
                DBG['DTI_P']['PRT_IND_P']['PI_P_TABLE']), 'Nombre del diseño', 'Nombre del prototipo')
        ...
        # prod nutracetico
        try:
            df = rename_col(clean_df(DBG['DTI_P']['NUTRA_P']['NUTRA_P_TABLE']),
                            'Nombre del producto', 'Nombre del producto nutracetico')
        ...
        try:
            df = rename_col(clean_df(DBG['DTI_P']['INN_GES_EMP_P']['IG_P_TABLE']),
                            'Nombre de la innovación', 'Nombre de la innovación empresarial')
        ...
        try:
            df = rename_col(clean_df(DBG['DTI_P']['INN_PROC_P']['IPP_P_TABLE']),
                            'Nombre de la innovación', 'Nombre de la innovación procesos y procedimientos')
        ...
        try:
            df = rename_col(clean_df(
                DBG['DTI_P']['REG_NORM_REGL_LEG_P']['RNR_P_TABLE']), 'Tipo producto', 'Nombre regulación')
        ...
        try:
            df = rename_col(clean_df(
                DBG['DTI_P']['EMP_BSE_TEC_P']['EBT_P_TABLE']), 'Tipo', 'Tipo de empresa base tecnologica')
        ...
        try:
            df = rename_col(clean_df(
                DBG['DTI_P']['EMP_CRE_CUL_P']['ICC_P_TABLE']), 'Empresa', 'Tipo de empresa base cultural')
        ...
        try:
            df = rename_col(clean_df(
                DBG['ASC_P']['PASC_P']['PASC_FOR_P_TABLE']), 'Nombre', 'Nombre producto interes social')
        ...
        # Proceso de apropiación social del conocimiento resultado del trabajo
        # conjunto
        try:
            df = rename_col(clean_df(DBG['ASC_P']['PASC_P']['PASC_TRA_P_TABLE']), 'Nombre',
                            'Nombre del Proceso de apropiación social del conocimiento resultado del trabajo conjunto entre un Centro de Ciencia y un grupo de investigación')
            ...
            print("Error: {}".format(e))
            pass
        # Nombre del Proceso de apropiación social del conocimiento para la
        # generación de insumos de política pública y normatividad
        try:
            df = rename_col(clean_df(DBG['ASC_P']['PASC_P']['PASC_GEN_P_TABLE']), 'Nombre',
        ...
        # Nombre del Proceso de apropiación social del conocimiento para el
        # fortalecimiento de cadenas productivas
        try:
            df = rename_col(clean_df(DBG['ASC_P']['PASC_P']['PASC_CAD_P_TABLE']), 'Nombre',
        ...
        # Piezas digitales
        try:
            df = rename_col(clean_df(DBG['ASC_P']['DC_P']['DC_CD_P_TABLE']), 'Título del proyecto',
                            'Título del proyecto para la generación de piezas digitales')
        ...
        # textuales
        try:
            df = rename_col(clean_df(DBG['ASC_P']['DC_P']['DC_CON_P_TABLE']), 'Título del proyecto',
                            'Título del proyecto para la generación de piezas Textuales (incluyendo cartillas, periódicos, revistas, etc.), Producción de estrategias transmediáticas y Desarrollos web')
        ...
        try:
            df = rename_col(clean_df(DBG['ASC_P']['DC_P']['DC_TRA_P_TABLE']),
                            'Título del proyecto', 'Título del proyecto estrategia trasmediatica')
        ...
        # desarrollo web
        try:
            df = rename_col(clean_df(DBG['ASC_P']['DC_P']['DC_DES_P_TABLE']),
                            'Título del proyecto', 'Título del proyecto desarrollo web')
        ...
        # tesis doctorado
        try:
            df = rename_col(clean_df(DBG['FRH_P']['TES_DOC_P']['TD_P_TABLE']),
                            'Título', 'Título de la tesis de doctorado')  # ,veh = 2

            eh = DBEH['FRH_P']['TES_DOC_P']['TD_P_TABLE']

            format_df(df, '12.Formación y programas',
                      var_w12, writer, eh, veh=2)
        ...
        # tesis maestria
        try:
            df = rename_col(clean_df(DBG['FRH_P']['TES_MAST_P']['TM_P_TABLE']),
                            'Título', 'Título del trabajo de grado de maestría')  # veh = 2
        ...
        # tesis pregrado
        try:
            df = rename_col(clean_df(DBG['FRH_P']['TES_PREG_P']['TP_P_TABLE']),
                            'Título', 'Título del trabajo de grado de pregrado')  # veh = 2
        ...
        try:
            df = rename_col(clean_df(
                DBG['FRH_P']['ASE_PRG_ACA_P']['APGA_P_TABLE']), 'Tipo', 'Nombre programa academico creado')
        ...
        try:
            df = rename_col(clean_df(
                DBG['FRH_P']['ASE_CRE_CUR_P']['ACC_P_TABLE']), 'Tipo', 'Nombre curso creado')
        ...
        try:
            df = rename_col(clean_df(
                DBG['FRH_P']['ASE_PRG_ONDAS_P']['APO_P_TABLE']), 'Integrante', 'Integrante programa ondas')
...
def checkpoint(DIR='InstituLAC', CHECKPOINT=True):
    ...
    try:
        oldend = len(DB) - 1
        if (dfg.loc[oldend]['Nombre del grupo'] == DB[oldend]['Info_group']['Nombre Grupo'].dropna().iloc[-1] and CHECKPOINT):
            start = oldend + 1
            return DB, dfg, start
